...
# ── Стандартные классы COCO (для базовой модели yolo26n.pt) ──
COCO_NAMES = {
    0: "человек",
    1: "велосипед",
    2: "автомобиль",
...
# ── Классы для дообученной модели обнаружения воздушных целей ──
# Используются при дообучении на датасетах Anti-UAV / DroneDetect
# Модель: yolo26n-uav.pt (дообученная)
UAV_NAMES = {
    0: "FPV-дрон",
    1: "развед. дрон",
    2: "ударный дрон",
...
# ── Приоритет угрозы по классам (выше = опаснее) ──
THREAT_PRIORITY = {
    "FPV-дрон": 10,        # Максимальный приоритет — FPV камикадзе
    "ударный дрон": 9,
    "ракета": 9,
    "развед. дрон": 7,
    "БПЛА крыло": 6,
    "вертолёт": 5,
    "самолёт": 4,
    "птица": 1,            # Ложная цель — минимальный приоритет
    "человек": 3,
}

# Активная таблица имён (переключается при загрузке модели)
_active_names = COCO_NAMES


def get_class_name(class_id: int) -> str:
    """Получить русское название класса."""
    return _active_names.get(class_id, f"класс_{class_id}")


def get_threat_priority(class_id: int) -> int:
    """Получить приоритет угрозы для класса (0-10, выше = опаснее)."""
    name = get_class_name(class_id)
    return THREAT_PRIORITY.get(name, 0)
